...
- **File-integrity checking** — verifies that every file listed in the manifest exists on disk and that the primary installer's `ProductVersion` matches the version recorded in `games.yaml`
- **Disk-size display** — shows the size of each game's installer folder at a glance
- **Player name & server IP** — games that opt in can accept a player name and/or a server IP address as MSI properties at install time
...
- Installer files placed under `Installers/<Game Name>/game/` (MSI or Inno Setup)
...
| `version` | string | Expected `ProductVersion` (used for integrity check) |
| `install_exe` | string | Primary executable/MSI path relative to `base_path` |
| `supports_player_name` | bool | Passes `PLAYERNAME` to the installer when `true` |
| `requires_server_ip` | bool | Prompts for a server IP and passes octets to the installer |
| `prerequisites` | list | Prerequisite installers to run before the main one |
| `files` | list | Files to check for integrity verification |
...
games:
  - name: Battlefield 4
    ...
    prerequisites: []
    files:
      - path: game/Battlefield 4-1.bin
      - path: game/Battlefield 4.exe
```
...
4. Edit `config\filter.yaml` to show only the games relevant to the event.
5. Run `Cobra LANs.exe`.
...
The script reads MSI metadata via the Windows Installer COM object and EXE version info via PowerShell. Manual fields (`supports_player_name`, `requires_server_ip`, `prerequisites`) are preserved across runs.
